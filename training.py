...
train, test = data_lp_rdd.randomSplit([0.7, 0.3], seed=21)

RF = RandomForest.trainClassifier(train, numClasses=10, categoricalFeaturesInfo={},
                                       numTrees=21, featureSubsetStrategy="auto",
                                       impurity='gini', maxDepth=30, maxBins=32)

pred = RF.predict(test.map(lambda x: x.features))

pred_rdd = test.map(lambda y: y.label).zip(pred)
...
test_error = pred_rdd.filter(
    lambda y: y[0] != y[1]).count() / float(test.count())
print('Test Error : ' + str(test_error))

RF.save(sc, 's3://aws-logs-658611668728-us-east-1/elasticmapreduce/j-2TVP4ZNS05GAU/trainingmodel.model')